...
if __name__ == '__main__':
    #边缘掩膜的提取
    model = BDCN()
    model.load_state_dict(torch.load('bdcn-final-model/vgg16.pth'))
    img_path = 'Datasets/COCOMask/train/train_class/train2.0/COCO_train2014_000000000072.jpg'
    img = cv2.imread(img_path)
    print(img.shape)
    h, w, c = img.shape
    img = np.float32(cv2.resize(img, (128, 128))) / 255
    print(img.shape)
    # 将图像转为 PyTorch 张量，并进行预处理
    ...
    #  使用模型进行边缘检测，得到输出结果
    with torch.no_grad():
        edge = model(img_tensor)[0][0].cpu().numpy()
    #print(edge.shape)
    # 对输出结果进行后处理，并将结果可视化
    ...
    print(edge.shape)
    edge_new = np.transpose(edge, (1, 2, 0))
    print('New shape:', edge_new.shape)

    cv2.imwrite('Datasets/COCOMask/train/train_class/COCO_train2014_000000000073.png',edge_new)
