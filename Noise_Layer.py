...
def perspective(image,device,d=8):
    ...
    #data_warp: torch.tensor = kornia.warp_perspective(image.float(), M, dsize=(h, w)).to(device) 原
    data_warp: torch.tensor = kornia.geometry.transform.warp_perspective(image.float(), M, dsize=(h, w)).to(device)
