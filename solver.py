...
class Solver(object):
    ...
    def build_model(self):
        if self.dataset in ['train_mask']:
            ...
            self.net = torch.nn.DataParallel(self.net)
            #self.net.load_state_dict(torch.load('models/train1.0/Encoder_Decoder_Model_mask_ScreenShooting_best.pth'))
            self.net.load_state_dict(torch.load(self.model_save_dir+'/'+self.distortion+'/'+self.model_name+'_mask_'+str(self.embedding_epoch)+'.pth'))
    ...
    def test_embedding(self):
        ...
        self.net_ED.eval()
        for i, (data, m, num) in enumerate(data_loader):#num 对待嵌入的图像进行打乱排序，存储的是图片的名字即编号
            #print(num)
            #print(m)
            inputs, m = Variable(data), Variable(m.float()) #对于编号为1的图像，嵌入的消息m对应为消息矩阵的W[1]的长度为30的消息
            ...
            for j in range(Encoded_image.shape[0]):
                ...
                index = num[j]
                cv2.imwrite('Datasets/Recover/result_1/'+str(index)+'.png', I2)
    ...
    def train_mask(self):
        ...
        # train the Encoder_Decoder
            ...
            I1 = (inputs[t,:,:,:].detach().to('cpu').numpy()+1)/2*255 #原图
            I1 = np.transpose(I1,(1,2,0))
            I2 = (Encoded_image[t,:,:,:].detach().to('cpu').numpy()+1)/2*255 #编码图像，该处理用于后续的保存和显示
            I2 = np.transpose(I2,(1,2,0))
            I_no = (Noised_image[t,:,:,:].detach().to('cpu').numpy()+1)/2*255 #含噪声图像
            I_no = np.transpose(I_no,(1,2,0))
    ...
    def test_accuracy(self):
        ...
        for i, (data, m, num) in enumerate(data_loader_test):
            ...
            inputs, m = inputs.to(self.device), m.to(self.device)
            self.net_D.eval()
            Decoded_message = self.net_D(inputs) #解码出来的信息  
            decoded_rounded = Decoded_message.detach().cpu().numpy().round().clip(0, 1)#将解码后的消息限制在01
